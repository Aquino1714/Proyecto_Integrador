...
def badge_rol(rol: str):
    ...
    return ft.Container(
        content=ft.Text(rol or "—", size=11, color=color, weight=ft.FontWeight.W_600),
        padding=ft.Padding.symmetric(horizontal=10, vertical=4),
        border_radius=6,
        bgcolor=ft.Colors.with_opacity(0.12, color),
        border=ft.Border.all(1, ft.Colors.with_opacity(0.5, color)),
...
def tarjeta_reporte(rep, on_click=None, on_resolver=None):
    ...
    return ft.Container(
        ...
        content=ft.Column(
            spacing=8,
            controls=[
                ft.Row(controls=[badge_estado(rep.estado), badge_rol(rep.rol_nombre)], spacing=8),
                ft.Row(
                    controls=[
                        ft.Column(
                            spacing=2,
                            ...
                            controls=[
                                ft.Text(
                                    f"{rep.asunto} — {rep.empleado_nombre} (EMP-{rep.empleado_id:03d})",
                                    size=14,
                                ...
                                ft.Text(
                                    ...
                                    + ("..." if rep.descripcion and len(rep.descripcion) > 120 else ""),
                                    size=12,
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Row(
                    controls=[
                        ft.Text(
                            f"{rep.empleado_nombre} (EMP-{rep.empleado_id:03d})",
                            size=14,
                            weight=ft.FontWeight.BOLD,
                            color=TEXT_PRIMARY,
                        ),
    ...
    return ft.Container(
        ...
        content=ft.Column(
            ...
            controls=[
                ...
                ft.Divider(height=1, color=DIVIDER),
                ft.Text(
                    f"¿Eliminar el reporte de {rep.empleado_nombre}? "
...
def reportes_empleados_content(page: ft.Page):
    ...
    def _refrescar():
        ...
        lista_wrapper_ref.current.controls = [
            tarjeta_reporte(r, on_click=abrir_detalle, on_resolver=confirmar_resuelto)
            for r in nuevos
        ]
    ...
    def confirmar_resuelto(rep):
        ReportsEmpDAO().marcar_resuelto(rep.reporte_id)
        _refrescar()
        cerrar_modal()
    ...
    def confirmar_eliminar(reporte_id):
        ReportsEmpDAO().delete(reporte_id)
        _refrescar()
        cerrar_modal()
            ...
            ft.Container(
                ...
                content=ft.Column(
                    controls=[
                        ...
                        ft.Text(
                            "Reportes enviados por choferes, recepción, almacén y trituración.",
                            size=12,
                            color=TEXT_SECONDARY,
                        ),
                        ft.Container(height=6),
